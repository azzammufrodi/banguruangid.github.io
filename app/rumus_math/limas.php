...
<body class="bg-slate-200">
  ...
  <div class="items-center p-5">
    <div class="card-body">
      <h4 class="font-bold text-4xl text-center text-gray-800 drop-shadow-sm">RUMUS LIMAS</h4>
    </div>
    <!-- INPUT -->
    <form action="" method="POST"
      class="flex flex-col items-center lg:flex lg:flex-col  md:flex md:flex-col md:items-center lg:items-center py-2">
      <p class="py-2 text-base text-gray-600 drop-shadow-sm">Masukan Nilai</p>
      <input
        class="shadow-sm m-2 p-2 border-2 border-stone-500 focus: outline-none focus:border-sky-500 focus:ring-sky-200 focus:ring-1 rounded-md text-center placeholder:text-gray-300"
        type="text" name="nilai_a" placeholder="Nilai Rusuk" required>
      <input
        class="shadow-sm m-2 p-2 border-2 border-stone-500 focus: outline-none focus:border-sky-500 focus:ring-sky-200 focus:ring-1 rounded-md text-center placeholder:text-gray-300"
        type="text" name="nilai_b" placeholder="Nilai Tinggi Sisi" required>
      <input
        class="shadow-sm m-2 p-2 border-2 border-stone-500 focus: outline-none focus:border-sky-500 focus:ring-sky-200 focus:ring-1 rounded-md text-center placeholder:text-gray-300"
        type="text" name="nilai_c" placeholder="Nilai Tinggi Linmas" required>
      <input
        class="m-3 cursor-pointer drop-shadow-md w-52 h-auto p-2 rounded-md text-slate-100 bg-orange-400 hover:bg-orange-500 hover:border-orange-400 hover:ring-orange-300 hover:ring-1 hover:outline-none"
  ...
  <!-- ganerate values -->
  <?php 
    $nilai_alas =$_POST['nilai_a']; 
    $nilai_tinggi =$_POST['nilai_b']; 
    $nilai_ltinggi =$_POST['nilai_c']; 

    $jumlahluas =  ($nilai_alas * $nilai_alas) + (4 * ((1*$nilai_alas*$nilai_tinggi)/2));
    $jumlahkeliling = 4 * $nilai_tinggi ;
    $jumlahvolume = (1 * ($nilai_alas * $nilai_alas) * $nilai_ltinggi) / 3;
// PRINT
    // echo "LUAS PERMUKAAN LIMAS ADALAH :<b> $jumlahluas CM² </b>";
    // echo "<br><br>KELILING LIMAS ADALAH :<b> $jumlahkeliling CM² </b>";
    // echo "<br><br>VOLUME LIMAS ADALAH : <b> $jumlahvolume CM³ </b>";
// PRINT END
?>
  ...
  <div
    ...
    <div class=" w-auto lg:w-auto p-4 border-2 border-stone-500 rounded-lg  text-center shadow-md m-3">
      ...
      <!-- END Detail Button -->
      <h2 class="font-bold lg:text-3xl">LUAS PERMUKAAN LIMAS</h2>
      <H1 class=" text-xl lg:text-5xl md:text-2xl"><?=number_format($jumlahluas)?>cm²</H1>
    </div>
    <div class=" w-auto lg:w-auto p-4 border-2 border-stone-500 rounded-lg text-center shadow-md m-3 ">
      <h2 class="font-bold lg:text-3xl">KELILING LIMAS</h2>
      <H1 class=" text-xl lg:text-5xl md:text-2xl"><?=number_format($jumlahkeliling)?>cm²</H1>
    </div>
    <div class=" w-auto lg:w-auto p-4 border-2 border-stone-500 rounded-lg text-center shadow-md m-3">
      <h2 class="font-bold lg:text-3xl">VOLUME LIMAS</h2>
